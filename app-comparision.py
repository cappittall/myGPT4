...
pd.DataFrame()
# Path to weights 
BASE_PATH = '/home/cappittall/Documents/myGPT4/GPT4All/bin'


esvar = open('.env', 'r').read().splitlines() 
for k in esvar:
    vars = k.split(':')
...
with st.sidebar:
    ...
    option = st.radio('Choose your task', ['Base Gen', 'Creative', 'Summarization', 'Few Shot', 'Python'], index=4)
    models =  [*list(os.listdir(BASE_PATH)), 'OpenAI']
    model = st.radio('Choose your model', models, index=len(models)-1)
    st.write(model)
